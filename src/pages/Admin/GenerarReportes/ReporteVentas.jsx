...
const TODAS_LAS_COLUMNAS = [
  ...
  { key: 'cliente_email', header: 'Email Cliente' },
  { key: 'cliente_nit', header: 'NIT/CI' },
  { key: 'cliente_razon', header: 'Razón Social' },
  { key: 'total', header: 'Total (Bs.)' },
  { key: 'estado_display', header: 'Estado Venta' },
  { key: 'pago_estado', header: 'Estado Pago' },
  { key: 'envio_estado', header: 'Estado Envío' },
  { key: 'num_items', header: 'N° Items' },
  { key: 'vendedor', header: 'Vendedor' },
  { key: 'tienda', header: 'Tienda' },
  { key: 'direccion', header: 'Dirección Envío' },
];
...
const ReporteVentas = () => {
  ...
  const fetchDatos = useCallback(async (url = null, limit = 10) => {
    ...
    if (url) {
      ...
      if (filtros.fechaInicio) {
        params.append('fecha_venta__date__gte', format(filtros.fechaInicio, "yyyy-MM-dd"));
      }
      if (filtros.fechaFin) {
        params.append('fecha_venta__date__lte', format(filtros.fechaFin, "yyyy-MM-dd"));
      }
      ...
      params.append('ordering', '-fecha_venta');
      
      endpoint = `/ventas/admin-ventas/?${params.toString()}`;
    }
  ...
  const getDatosParaExportar = async () => {
    ...
    if (filtros.fechaInicio) params.append('fecha_venta__date__gte', format(filtros.fechaInicio, "yyyy-MM-dd"));
    if (filtros.fechaFin) params.append('fecha_venta__date__lte', format(filtros.fechaFin, "yyyy-MM-dd"));
    if (filtros.estado) params.append('estado', filtros.estado);
    params.append('ordering', '-fecha_venta');
    
    try {
      const response = await apiClient.get(`/ventas/admin-ventas/?${params.toString()}`);
  ...
  const renderCellContent = (dato, key, paraExportar = false) => {
    ...
    switch (key) {
      case 'id':
        return dato.id;
      case 'fecha_venta':
        return format(new Date(dato.fecha_venta), 'dd/MM/yyyy HH:mm');
      case 'cliente_nombre':
        return dato.cliente?.nombre_completo || na;
      case 'cliente_email':
        return dato.cliente?.email || na;
      case 'cliente_nit':
        return dato.cliente?.nit || blank;
      case 'cliente_razon':
        return dato.cliente?.razon_social || blank;
      case 'total':
        const totalNum = parseFloat(dato.total);
        return isNaN(totalNum) ? (paraExportar ? '0.00' : '0.00') : totalNum.toFixed(2);
      case 'estado_display':
        return dato.estado_display || na;
      case 'pago_estado':
        return dato.pagos?.[0]?.estado_display || na;
      case 'envio_estado':
        return dato.envio?.estado_display || na;
      case 'num_items':
        return dato.items?.length || 0;
      case 'vendedor':
        return dato.vendedor || blank;
      case 'tienda':
        return dato.tienda || na;
      case 'direccion':
        return dato.envio?.direccion_entrega || blank;
      default:
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 bg-gray-50 min-h-screen">
      ...
      <div className="p-4 sm:p-6 bg-white rounded-xl shadow-md mb-6 sm:mb-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Fecha Venta (Desde)</label>
            <DatePicker
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Fecha Venta (Hasta)</label>
            <DatePicker
          ...
          <div>
            ...
            <div className="relative">
              <input
                type="text"
                placeholder="Buscar por ID, cliente, email, NIT..."
