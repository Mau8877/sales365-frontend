...
const TODAS_LAS_COLUMNAS = [
  ...
  { key: 'objeto', header: 'Objeto Afectado' },
];

const COLUMNAS_DEFAULT = ['timestamp', 'user', 'accion', 'ip'];
...
const ReporteBitacora = () => {
  ...
  const fetchDatos = useCallback(async (url = null, limit = 10) => {
    ...
    if (url) {
      ...
      if (filtros.fechaInicio) {
        params.append('timestamp__gte', format(filtros.fechaInicio, "yyyy-MM-dd'T'HH:mm:ss"));
      }
      if (filtros.fechaFin) {
        params.append('timestamp__lte', format(filtros.fechaFin, "yyyy-MM-dd'T'HH:mm:ss"));
      }
      
      params.append('ordering', '-timestamp');
      
      endpoint = `/auditoria/bitacoras/?${params.toString()}`;
    }
  ...
  const getDatosParaExportar = async () => {
    ...
    if (filtros.search) params.append('search', filtros.search);
    if (filtros.fechaInicio) params.append('timestamp__gte', format(filtros.fechaInicio, "yyyy-MM-dd'T'HH:mm:ss"));
    if (filtros.fechaFin) params.append('timestamp__lte', format(filtros.fechaFin, "yyyy-MM-dd'T'HH:mm:ss"));
    params.append('ordering', '-timestamp');
    
    try {
      const response = await apiClient.get(`/auditoria/bitacoras/?${params.toString()}`);
  ...
  const handleExportPDF = async () => {
    ...
    const doc = new jsPDF();
    doc.text("Reporte de Bitácora", 14, 16);
  ...
  const renderCellContent = (dato, key, paraExportar = false) => {
    ...
    switch (key) {
      case 'timestamp':
        return format(new Date(valor), 'dd/MM/yyyy HH:mm:ss');
      case 'user':
        return valor ? valor.email : (paraExportar ? "Sistema" : <span className="italic text-gray-500">Sistema</span>);
      case 'tienda':
        return valor ? valor.nombre : (paraExportar ? "N/A" : <span className="italic text-gray-500">N/A</span>);
      case 'extra':
        return valor ? (paraExportar ? JSON.stringify(valor) : <code className="text-xs">{JSON.stringify(valor)}</code>) : '';
      default:
        return valor;
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div>
            ...
            <div className="relative">
              <input
                type="text"
                placeholder="Buscar en acción, objeto, usuario, IP..."
